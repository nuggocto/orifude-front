...
test('a published release exposes only reviewed instructions and escapes its notes', async ({ browser }) => {
  ...
  try {
    ...
    const posix = page.locator('summary').filter({ hasText: 'POSIX installer' });
    await posix.focus();
    ...
    await expect(page.locator('summary').filter({ hasText: 'Homebrew' })).toHaveCount(0);
    await expect(page.locator('summary').filter({ hasText: 'Scoop' })).toHaveCount(0);
    await expect(page.locator('summary').filter({ hasText: 'Arch Linux' })).toHaveCount(0);
    await page.locator('summary').filter({ hasText: 'PowerShell' }).click();
